...
import os, json, random, shutil, logging, math
...
def main() -> None:
    ...
    CFG: Dict[str, Any] = dict(
        seed=3407,
        dataset_path="simulation_data_31_least_inverse.json",
    ...
    # Derived paths
    OUTPUT_ROOT = "results_inverse_llama"
    ...
    train_ds = load_dataset(
        "json",
        data_files=str(output_dir / "train.json"),
        split="train",  # literal "train" because JSON has no native splits
    ).map(fmt, batched=True, remove_columns=["instruction", "input", "output"])
